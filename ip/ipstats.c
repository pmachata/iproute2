...
static enum ipstats_maybe_on_off ipstats_dissect_01(int value, const char *what)
{
	...
	default:
		fprintf(stderr, "Invalid value for %s: expected 0 or 1, got %d. Assuming 1.\n",
			what, value);
				    ...
				    const struct ipstats_stat_desc *desc)
{
	...
	stats = IPSTATS_RTA_PAYLOAD(struct mpls_link_stats,
				    mrtb[MPLS_STATS_LINK]);
	if (stats == NULL) {
		fprintf(stderr, "Error: attribute payload too short");
		return -EINVAL;
	}
		     ...
		     struct ipstats_stat_enabled *enabled)
{
	...
	for (i = 0; i < enabled->nenabled; i++) {
		...
		print_int(PRINT_ANY, "ifindex", "%d:", show_attrs.ifsm->ifindex);
		print_color_string(PRINT_ANY, COLOR_IFNAME, "ifname", " %s:", dev);
