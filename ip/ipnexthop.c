...
static void print_nh_res_group(FILE *fp, const struct rtattr *res_grp_attr)
{
	...
	if (tb[NHA_RES_GROUP_BUCKETS])
		print_uint(PRINT_ANY, "buckets", "buckets %u ",
			   rta_getattr_u32(tb[NHA_RES_GROUP_BUCKETS]));
...
static void print_nh_res_bucket(FILE *fp, const struct rtattr *res_bucket_attr)
{
	...
	if (tb[NHA_RES_BUCKET_INDEX])
		print_uint(PRINT_ANY, "index", "index %u ",
			   rta_getattr_u32(tb[NHA_RES_BUCKET_INDEX]));
				    ...
				    char ***argvp)
{
	...
	while (argc > 0) {
		if (strcmp(*argv, "buckets") == 0) {
			...
			addattr32(n, maxlen, NHA_RES_GROUP_BUCKETS, buckets);
...
static int ipnh_bucket_get_id(__u32 id, __u32 bucket_index)
{
	struct {
		struct nlmsghdr	n;
		struct nhmsg	nhm;
	...
	addattr32(&req.n, sizeof(req), NHA_RES_BUCKET_INDEX, bucket_index);
...
static int ipnh_bucket_get(int argc, char **argv)
{
	...
	while (argc > 0) {
		if (!strcmp(*argv, "id")) {
			...
			NEXT_ARG();
			if (get_unsigned(&bucket_index, *argv, 0))
